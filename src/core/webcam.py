...
from src.core.motion_tracker import (
    calculate_hand_motion, detect_hand_drift, calculate_dynamic_threshold,
    is_motion_gating_active, compute_transition_movement
)
from src.core.landmark_processor import (
    landmarks_to_numpy, landmarks_to_bbox, wrist_point_px, detect_person_boxes
)
from src.ui.renderer import draw_landmarks, CYAN, ORANGE, BLUE, RED, GREEN, WHITE, YELLOW, BLACK
    ...
    while True:
        ...
        with profile_section("hand_detection"):
            ...
            if not HOG_ENABLED or cfg.preprocessing.disable_hog_detection or hog_detector is None:
                ...
                people = detect_person_boxes(frame, hog_detector)
        ...
        if detect_hand_drift(hand_infos, wrist_history, frame.shape, HAND_DRIFT_MIN_JUMP_PX, HAND_DRIFT_JUMP_RATIO):
        ...
        with profile_section("hand_selection"):
            ...
            if hand_infos and len(hand_infos) <= 1:
                ...
                face_lms_np = landmarks_to_numpy(face_landmarks)  # (468, 3)
                hand_lms_list = [landmarks_to_numpy(info["landmarks"]) for info in hand_infos]  # List of (21, 3)
        ...
        for info in filtered_hand_infos:
            ...
            hand_box = landmarks_to_bbox(hand, w, h)
            ...
            wrist_points.append(wrist_point_px(hand, w, h))
        ...
        if wrist_points:
            ...
            motion_magnitude = calculate_hand_motion(avg_wrist, wrist_history, motion_magnitude, MOTION_SMOOTHING)
        ...
        if valid_for_prediction:
            transition_movement = compute_transition_movement(landmarks_vec, previous_landmarks_vec)
        ...
        if valid_for_prediction and len(sequence_buffer) == NUM_FRAMES:
            ...
            effective_threshold = calculate_dynamic_threshold(motion_magnitude, prediction_stability_counter, is_transition, CONFIDENCE_THRESHOLD, DYNAMIC_THRESHOLD_ENABLED, TRANSITION_HYSTERESIS, MOTION_THRESHOLD, MOTION_BOOST_FACTOR, STABILITY_BOOST_FACTOR, DYNAMIC_THRESHOLD_MIN)

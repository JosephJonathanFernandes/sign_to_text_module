...
* **July 14**: A full-day engineering sprint focused on evaluation, the active learning pipeline, and emergency detection infrastructure. Built and wired the complete feedback loop: the `/feedback` WebSocket endpoint captures user corrections, which trigger background adapter training via `AdapterTrainingManager`, allowing the model to personalize to individual signers without retraining the base Bi-GRU. Implemented robust per-connection `InferenceSession` state management with UUID-keyed isolation. Architected and deployed the Emergency Sign Detection module (`api/emergency.py`) — a configurable keyword monitor that fires real-time alerts via ntfy push notifications and local logs when distress signs are detected above a confidence threshold. Built a comprehensive automated WebSocket evaluation suite (`src/evaluation/`) measuring end-to-end latency, stress throughput, and system stability over extended runs. This suite produced the empirical benchmarks: ONNX inference at 105 FPS / 9.1ms mean latency on consumer CPU hardware, and a stability coefficient of variation below 5% over extended sessions. Fixed multiple bugs in the evaluation pipeline: Pydantic 422 payload schema errors, WebSocket JSON payload format mismatches, session `pending_count` attribute initialization, and `AdapterModel` state dict unpacking. Regenerated dataset heuristics (`keypoints.csv`, `candidate_map.json`, `confidence_statistics.json`) to reflect the updated 152-class vocabulary. All results written to `results/` directory and committed.
* **July 15**: A focused API hardening and robustness engineering day. Morning: conducted a full 12-point backend production-readiness audit (`results/api_audit/production_readiness_audit.md`) covering model lifecycle, input validation, temporal pipeline, WebSocket handling, performance, API design, configuration, robustness, security, code quality, and production readiness. Resulting in three immediate fixes committed under `chore(api)`: (1) a non-fatal `np.isfinite()` NaN/Inf frame validation that drops corrupted MediaPipe frames without disconnecting the session, (2) a `MAX_PAYLOAD_SIZE = 50,000` bytes payload guard that closes the connection with WebSocket code 1008 on oversized payloads, and (3) extraction of all magic numbers (`0.15`, `"adapter_weights"`) to named constants at the top of `api/app.py`. Ran the automated smoke test suite to validate all four scenarios: normal use, abrupt disconnect, NaN payload, and oversized payload — all passed. Tagged the repo `v2.0.0-final` as the feature-frozen submission build. Afternoon: Conducted a definitive asyncio architecture analysis (`results/api_audit/queue_architecture_analysis.md`) using an exact execution trace, proving that the current synchronous `await run_in_executor` design is correct — inference at ~10ms outpaces the 33ms frame interval, so an asyncio.Queue refactor would introduce concurrency risk without solving any observable bottleneck. Compiled full Architecture Decision Records (`docs/ARCHITECTURE_DECISIONS.md`) documenting 9 implemented decisions and 4 explicitly rejected alternatives with technical justifications. Evening: Conducted a skeleton quality estimation engineering audit (`results/api_audit/skeleton_quality_audit.md`), discovering that the offline dataset curation tool (`quality_filter_hybrid.py`) already implements 6 of the 7 proposed quality checks, but none are wired into the live inference path. Based on this, implemented two targeted quality gates in `api/app.py` (`feat(api)` commit): (1) a zero-ratio check that drops frames where >50% of raw hand coordinates are zero (MediaPipe missed both hands), and (2) a landmark jump detector that compares consecutive frames and drops anomalous tracking glitches using a session-level `landmark_jump_count` counter — mirroring the existing `idle_frames` pattern. Fixed a logic bug in the same session (`fix(api)` commit): `session.prev_frame` was incorrectly updated on rejected frames, which allowed masked drift by comparing each bad frame against the previous bad frame instead of the last known-good position. Added `scratch/calibrate_jump_threshold.py` to empirically measure the jump distribution from real signing data and validate the 0.20 threshold before final submission. Also resumed vocabulary expansion: processed the new `better` sign class through the full pipeline — augmentation, quality filtering, merge, and heuristics regeneration.
* **July 16-17 (Planned)**: Final Presentation (PPT) preparation and empirical threshold calibration from real-world signing sessions. Validate the model's sign predictions against official Goa Board ISL videos and Muskaan Ma'am's ISL book.

...
Furthermore, a comprehensive 2.0-minute continuous streaming stability test recorded a negligible memory growth of 0.02 MB, confirming zero memory leakage during high-throughput execution. The WebSocket backend API survived 100% of adversarial fault tolerance injections (including malformed JSON, NaN spatial coordinates, and invalid dimensional payloads) with zero backend degradation.
...
*Empirical Constraints and Assumptions:* It is critical to acknowledge that all backend latency evaluations (averaging 29.42 ms round-trip) were conducted over a local loopback interface and do not account for external internet routing delays. Furthermore, while the backend API proved highly resilient to synthetic asynchronous load testing (sustaining up to 50 concurrent connections), these figures isolate backend processing capacity and do not model the client-side memory or DOM-rendering overhead inherent to browser-based MediaPipe extraction. Finally, the continual learning adapter was evaluated using programmatic spatial drift simulations; real-world efficacy will remain heavily dependent on individual camera positioning, lighting variance, and human articulatory noise.

*Accuracy Variance Justification:* While early laboratory validation on strictly controlled subsets yielded a 98.33% accuracy, the final robust evaluation—encompassing all 92,519 cross-domain samples—stabilized at 94.13%. This variance is heavily intentional; the robust evaluation dataset purposefully introduces a `__reject__` class comprising 3,915 sequences of pure motion epenthesis and random non-sign noise. Achieving a 94.13% overall accuracy (and a 95.81% Macro F1) in a dense 301-class environment that actively tests against motion hallucinations provides a vastly more credible benchmark for real-world viability than sterile laboratory subsets.
